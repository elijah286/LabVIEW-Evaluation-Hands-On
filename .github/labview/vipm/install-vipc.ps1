...
$PublicRepoUrl    = if ($Env:VIPM_PUBLIC_REPO_URL) { $Env:VIPM_PUBLIC_REPO_URL } else { 'https://github.com/elijah286/LabVIEW-Evaluation-Hands-On.git' }
...
function Save-PublicVipmPackage($Package, [string] $OutDir) {
    ...
    Invoke-WebRequest -Uri $url -OutFile $outFile -UseBasicParsing -MaximumRedirection 10 -TimeoutSec 300 -Headers @{ 'User-Agent' = 'LabVIEW-Evaluation-Hands-On VIPM downloader' } | Out-Null
